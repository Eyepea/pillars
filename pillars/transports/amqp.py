...
class AppRunner(BaseRunner):
    ...
    async def _make_server(self) -> "AmqpServer":
        return AmqpServer(self._app._handler)
...
class AmqpServer:
    def __init__(self, handler: Callable[[dict], Awaitable[None]]) -> None:
        ...
        self._connections: List["AmqpProtocol"] = list()

    def __call__(self) -> "AmqpProtocol":
        proto = AmqpProtocol(handler=self._handler)
